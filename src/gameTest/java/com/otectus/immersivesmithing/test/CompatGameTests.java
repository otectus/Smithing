...
public final class CompatGameTests {
    ...
    public static void autoDetectionAgainstLoadedMods(GameTestHelper h) throws Exception {
        ...
        ImmersiveSmithing.LOGGER.info("Compat test: {} automatic recipes; {}", auto.size(), data.report().summary());
        h.succeed();
    }
    ...
    public static void spartanWeaponryIsSmithedByDefault(GameTestHelper h) {
        ...
        for (String type : SPARTAN_TYPES) {
            for (Map.Entry<String, String> metal : SPARTAN_METALS.entrySet()) {
                ...
                h.assertTrue(recipes.size() == 1 && recipes.get(0).getId().equals(ImmersiveSmithing.id("compat/spartanweaponry/" + name)),
                ...
                h.assertTrue(r.explicitAnvilPattern() != null && r.explicitAnvilPattern().equals(ImmersiveSmithing.id(type)), "Dedicated anvil pattern for " + name);
        ...
        h.assertTrue(EquipmentClassifier.classify(new ItemStack(longsword)).equals(ImmersiveSmithing.id("longsword")), "Longsword pattern by tag");
